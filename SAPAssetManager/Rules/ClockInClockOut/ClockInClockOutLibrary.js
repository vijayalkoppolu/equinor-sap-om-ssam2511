        ...
        if (libThis.isCICOEnabled(context) && collection && collection.length > 0) {
            ...
            if (sortUserTimeEntry.PreferenceGroup === 'CLOCK_IN') {
                if (businessObjectType === '#sap_mobile.MyWorkOrderHeader') {
                    ...
                    return sortUserTimeEntry.OrderId && sortUserTimeEntry.OperationNo && (ValidationLibrary.evalIsEmpty(sortUserTimeEntry.CapacityRecordCounter) || sortUserTimeEntry.CapacityRecordCounter === '0000');
                } else if (businessObjectType === '#sap_mobile.MyWorkOrderOperationCapacityRequirement') {
                    return sortUserTimeEntry.OrderId && sortUserTimeEntry.OperationNo && sortUserTimeEntry.CapacityRecordCounter && sortUserTimeEntry.CapacityRecordCounter !== '0000';
                }
                ...
                if (results && results.length > 0) {
                    ...
                    if (row.PreferenceGroup === 'CLOCK_IN') {
                        ...
                        if (libMobile.isOperationStatusChangeable(context) && row.WOOperation_Nav && row.WOOperation_Nav.OperationMobileStatus_Nav && libThis.allowClockInOverride(context, row.WOOperation_Nav.OperationMobileStatus_Nav.MobileStatus)) {
                            ...
                            if (isSyncInProgress(context) && ODataLibrary.isLocal(row)) {
                                return Promise.resolve(true);
                            } else {
                                let opNum = '';
                                if (row.OperationNo) {
        ...
        return context.executeAction({
            'Name': '/SAPAssetManager/Actions/ClockInClockOut/WorkOrderClockInOut.action', 'Properties': {
                ...
                'Headers': {
                    ...
                    'OfflineOData.TransactionID': binding.ClockOrderId,
                },
    ...
    static getLinkProperties(context, odataType) {
        switch (odataType) {
            ...
            case context.getGlobalDefinition('/SAPAssetManager/Globals/ODataTypes/WorkOrderOperationCapacity.global').getValue():
                return {
                    linkEntitySet: 'MyWorkOrderOperationCapacityRequirements',
                    linkProperty: 'WOOperCapacityRequirement_Nav',
                };
            default:
                return {
