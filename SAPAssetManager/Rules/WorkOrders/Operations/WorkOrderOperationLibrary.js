...
import StandardTextPickerIsVisible from './CreateUpdate/StandardTextPickerIsVisible';
import ConstantsLibrary from '../../Common/Library/ConstantsLibrary';
import { ChangeExistingDescriptionTypes } from './CreateUpdate/ChangeExistingDescriptionPickerItems';
...
export class OperationEventLibrary {
    ...
    static addNoteAfterUpdate(pageProxy, updateAction) {
        const shouldChangeNote = OperationEventLibrary.isStandardTextWasChanged(pageProxy);
        CommonLibrary.setStateVariable(pageProxy, 'skipToastAndClosePageOnDocumentCreate', !!shouldChangeNote);  // if there is an STK in the note control (not visible), dont close the page before creating the note
        return pageProxy.executeAction(updateAction)
            .then(() => {
                ...
                libCommon.setStateVariable(pageProxy, ConstantsLibrary.stripNoteNewTextKey, OperationEventLibrary.replaceNoteByStandardText(pageProxy));
    ...
    static isStandardTextWasChanged(pageProxy) {
        try {
            if (StandardTextPickerIsVisible(pageProxy)) {
                const selectedStandardTextKeyValue = libCommon.getControlValue(libCommon.getControlProxy(pageProxy, 'StandardTextKeyListPicker'));
                const bindStandardTextKeyValue = pageProxy.binding.StandardTextKey;
                return selectedStandardTextKeyValue !== bindStandardTextKeyValue;
            }

            return false;
        } catch (error) {
            Logger.error('isStandardTextWasChanged', error);
            return false;
        }
    }

    static replaceNoteByStandardText(pageProxy) {
        try {
            if (StandardTextPickerIsVisible(pageProxy)) {
                const changeExistingDescriptionSegmentedValue = libCommon.getControlValue(libCommon.getControlProxy(pageProxy, 'ChangeExistingDescriptionSegmented'));
                return changeExistingDescriptionSegmentedValue === ChangeExistingDescriptionTypes.replace;
            }

            return false;
        } catch (error) {
            Logger.error('replaceNoteByStandardText', error);
            return false;
        }
    }
}
