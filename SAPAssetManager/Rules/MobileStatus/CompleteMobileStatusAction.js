                ...
                case 'MyWorkOrderOperations': {
                    switch (bindingObj['@odata.type']) {
                        ...
                        case '#sap_mobile.MyWorkOrderOperationCapacityRequirement': {
                            mobileStatusObject = bindingObj.PMMobileStatus_Nav;
                            mobileInstance = bindingObj;
                            headerObj = mobileInstance.MyWorkOrderOperation_Nav.WOHeader;
                            break;
                        }
                        default:
                            break;
        ...
        if (libCommon.isDefined(mobileStatusObject)) {
            ...
            return libSuper.checkReviewRequired(context, target).then(review => {
                ...
                return libClock.reloadUserTimeEntries(context, false, false, mobileInstance).then(() => {
                    return libClock.clockOutBusinessObject(context, mobileInstance).then(async () => {
                        if (review) { //target requires review for technician user
                            return context.executeAction({
                                ...
                                'Properties': {
                                    ...
                                    'Headers': {
                                        'OfflineOData.TransactionID': headerObj.OrderId,
                                    },
                                },
                            });
                        }
                        ...
                        const acceptedStatus = await context.read('/SAPAssetManager/Services/AssetManager.service', 'EAMOverallStatusConfigs', ['EAMOverallStatus'], `$filter=MobileStatus eq '${completeMobileStatus}' and ObjectType eq '${eamObjType}' and EAMOverallStatusProfile eq '${mobileStatusObject.EAMOverallStatusProfile}'`);
                        const properties = {
                            ...
                            'EAMOverallStatus': acceptedStatus.getItem(0).EAMOverallStatus || '',
                        };
